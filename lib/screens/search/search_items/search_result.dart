...
class SearchResults extends StatelessWidget {
  ...
  const SearchResults({Key? key, required this.searchText, required this.cubit})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    List<UserModel> contactsPhoneResult = cubit.users.where((element) {
      return element.phone!.toLowerCase().contains(searchText.toLowerCase());
    }).toList();

    List<UserModel> chatsPhoneResult = cubit.chats.where((element) {
      return element.phone!.toLowerCase().contains(searchText.toLowerCase());
    }).toList();

    List<UserModel> totalResult =
        (nameResult + contactsPhoneResult + chatsPhoneResult).toSet().toList();
    // totalResult.
